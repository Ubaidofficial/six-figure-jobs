...
// Recruitee-tagged wrapper around the shared scraper fetch helper. Previous
// policy ignored Retry-After; the shared util honors it on 429/503 and
// applies exponential backoff for transient 5xx/network errors.
async function fetchJsonWithRetry<T>(
  ...
  return fetchJsonWithBackoff<T>(url, {
    ...
    onRetry: (info) =>
      console.warn(
        `[Recruitee] retry ${info.attempt}/${info.attempts} for ${info.url} in ${info.delayMs}ms (${info.reason})`,
      ),
  })
}
