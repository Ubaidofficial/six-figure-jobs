...
export async function scrapeRemoteOtter(): Promise<ScraperStats> {
  console.log('[RemoteOtter] Starting scrape...')
  ...
  try {
    ...
    for (const el of cards) {
      ...
      const company =
        card.find('[data-role="company"], .company').first().text().trim() || 'Unknown company'
      ...
      if (company && isExternalToHost(applyUrl, 'remoteotter.com')) {
    ...
    console.log(`[RemoteOtter] ✓ Scraped ${stats.created} jobs`)
    return stats
  } catch (error) {
    console.error('[RemoteOtter] ❌ Scrape failed:', error)
