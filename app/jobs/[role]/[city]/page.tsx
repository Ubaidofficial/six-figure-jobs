...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const canonical = `${SITE_URL}/jobs/${role}/${cityInfo.slug}`
...
export default async function RoleCityPage({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 4, name: `${cityInfo.label} jobs`, item: `${SITE_URL}/jobs/${role}/${cityInfo.slug}` },
    ],
  }
  ...
  const faqJsonLd = {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: 'All listings are $100k+; many senior roles exceed $200k. Exact ranges depend on company, level, and stack fit.',
        },
      },
      {
        '@type': 'Question',
