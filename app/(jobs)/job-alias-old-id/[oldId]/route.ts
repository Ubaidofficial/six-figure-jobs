...
export async function GET(request: Request, ctx: { params: Promise<{ oldId: string }> }) {
  const { oldId } = await ctx.params

  const job = await findJobByIncomingSlug(oldId)
...
export async function HEAD(request: Request, ctx: { params: Promise<{ oldId: string }> }) {
